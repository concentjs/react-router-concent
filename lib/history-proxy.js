...
function _callHistoryMethod() {
  ...
  var method = args[0]; // 用户透传的调用参数 arguments 伪数组，这里需提前转为真数组

  var inputArgs = slice.call(args[1]); // hash 路由不支持透传 state，不能如下方式
...
export function getLatestCallInfo() {
  return latestInfo;
}
export function clearLatestCallInfo() {
  latestInfo = {
    callByApi: false,
    callTime: 0
  };
}
export function setHistory(_history) {
  history = _history;
}
